...
import test.java.template.handler.*
...
class TemplateHandlerTest : BaseTest() {
    ...
    fun `cast object to list`() {
        ...
        val impl = getJavaDaoInstance<CastObjectToListDao>()
        val list = impl.query(listOf(0, 0))
        assert(list.size == 1)
        assertContentEquals(list.map { it.id }, listOf(1))
    }
    ...
    fun `boxed primitive type parameter`() {
        ...
        val impl = getJavaDaoInstance<BoxedPrimitiveTypeParameterDao>()
    ...
    fun `boxed primitive type property`() {
        ...
        val impl = getJavaDaoInstance<BoxedPrimitiveTypePropertyDao>()
    ...
    @Test
    fun `error, duplicate property declared`() {
        compileFailure<DuplicatePropertyDeclaredBadDao> {
            assert(
                errorMessages.contains("Property 'i' already exists")
            )
        }
    }
    @Test
    fun `error, property is not a map`(){
        compileFailure<PropertyIsNotMapBadDao> {
            assert(
                errorMessages.contains("id is not a map")
            )
        }
    }
    @Test
    fun `error, property is not a list`(){
        compileFailure<PropertyIsNotListBadDao> {
            assert(
                errorMessages.contains("id is not a list")
            )
        }
    }
    @Test
    fun `error, missing property`(){
        compileFailure<MissingPropertyBadDao> {
            assert(
                errorMessages.contains("error expr:person.company, missing property:test.Person.company.")
            )
        }
    }
    @Test
    fun `error, expr type cast error`(){
        compileFailure<ExprTypeCastErrorBaoDao> {
            assert(
                errorMessages.contains("a.b is of type java.lang.String cannot assignable to java.lang.Integer")
            )
        }
    }
